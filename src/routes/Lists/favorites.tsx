...
import { IQueryFavoriteListGet, QUERY_FAVORITE_LIST_GET } from '../../../Data/queries';
...
class FavoriteList extends React.Component<IListProps, IListStates> {
  ...
  public queryHandler = async () => {
    ...
    try {
      const favoriteListGet = await client.query<IQueryFavoriteListGet>({
        query: QUERY_FAVORITE_LIST_GET,
      ...
      const { data } = favoriteListGet;

      if (data) {
        data.favoriteListGet.map((item: any) => {
        ...
        this.setState({
          queryLoading: false,
          result:
          data.favoriteListGet.map((item: any, index: number) =>
            <TableEntry
...
export default withApollo(FavoriteList);
